...
launcher_bootstrap "Oracle Operator GUI" "$REPO_DIR"

export NEJE_UPLOADER_SESSION_ROOT="$SCRIPT_DIR"
if [[ -z "${NEJE_PLOTTER_SPOOL_ROOT:-}" ]]; then
  export NEJE_PLOTTER_SPOOL_ROOT="$REPO_DIR/spool"
fi

launcher_ensure_dir_var "NEJE_UPLOADER_SESSION_ROOT" "This should be the TouchDesigner sessions folder."
launcher_ensure_dir_var "NEJE_PLOTTER_SPOOL_ROOT" "Run setup or set this in $REPO_DIR/.env."

echo "GUI:             http://${NEJE_GUI_HOST:-127.0.0.1}:${NEJE_GUI_PORT:-8787}/"
echo "Sessions folder: $NEJE_UPLOADER_SESSION_ROOT"
echo "Spool folder:    ${NEJE_PLOTTER_SPOOL_ROOT:-$REPO_DIR/spool}"
echo
