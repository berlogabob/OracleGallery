    ...
    with ui.column().classes("workspace-scroll gap-2"):
        ...
        with ui.card().classes("oracle-card compact-card w-full"):
            ...
            with ui.grid(columns=2).classes("w-full gap-2"):
                ...
                number_control(fields, "z_up_mm", label="Pen-up Z (mm)", value=settings.z_up_mm, default=0, min_value=-25, width_class="w-full", tooltip="Legacy absolute Z-up value; current pen-up behavior uses Z homing.", on_change=persist_and_refresh)
                number_control(fields, "z_down_mm", label="Pen-down Z (mm)", value=settings.z_down_mm, default=-25, min_value=-25, width_class="w-full", tooltip="Legacy value; current pen-down behavior is fixed at absolute G0 Z-25.", on_change=persist_and_refresh)
