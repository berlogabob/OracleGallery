...
from .origin_markers import ORIGIN_FILLER_MACBOOK, tags_for_origin
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Generate test Oracle session folders or filler symbols.")
    parser.add_argument("--mode", choices=["user", "idle", "filler"], default="user")
...
def generate_filler_session_packages(
    *,
    source_root: Path,
    output_root: Path,
    scale_config: Path,
    ...
    for index in range(count):
        ...
        mark_index = symbols.index(base_symbol) % len(MARK_NAMES)
        mark_name = MARK_NAMES[mark_index]
        oracle_text = "Local filler mark for empty sheet cells."
        themes = ["FILLER", "LOCAL", "MACBOOK"]
        scale = scales.get(base_symbol.name, 1.0) * global_scale
            ...
            json.dumps(
                {
                    ...
                    "tags": tags_for_origin(ORIGIN_FILLER_MACBOOK),
                    "baseSymbol": base_symbol.name,
                    "symbolScale": scale,
                    "kind": "filler",
...
def _unique_session_id(output_root: Path, now: datetime, index: int, *, prefix: str = "test") -> str:
    base = now.strftime(f"{prefix}_%Y%m%d_%H%M%S")
