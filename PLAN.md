...
8. В `EXHIBITION REAL` после successful preflight нажать `Arm Real`, затем `Start Print`.
9. Для обычной остановки использовать `Stop After Sheet`; это не emergency stop и не рвёт текущий row G-code stream. Для аварийной остановки использовать `Emergency Stop` и физическую кнопку питания/аварийный стоп.
...
- **Row preview depth:** добавить визуальное выделение текущего ряда и последнего успешно отправленного row G-code в preview.
