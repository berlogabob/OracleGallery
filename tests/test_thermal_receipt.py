...
def test_thermal_payload_from_real_session_has_mandatory_contract() -> None:
    ...
    payload = send_receipt.build_payload(
        session_dir=REPO_ROOT / "assets" / "sessions" / "20260518_154452",
        repo_root=REPO_ROOT,
        include_symbol=True,
        printer_width=384,
        ilabel_width=576,
    ...
    assert payload["session_url"].endswith("/#/session/20260518_154452")
...
def test_printer_connect_receipt_dry_run_outputs_payload_and_preview(tmp_path: Path) -> None:
    ...
    result = subprocess.run(
        [
            ...
            "receipt",
            str(REPO_ROOT / "assets" / "sessions" / "20260518_154452"),
            "--dry-run",
            "--preview-png",
            str(preview),
        ],
