...
- **Pattern generator (`SKETCH` mode)** — an embedded p5.js sketch (`echodraw/generative-core/web/sketch.js`) with a 15-generator library (`circles`, `waves`, `gridwalk`, `flowfield`, `mondrian`, `tribal`, `circuit`, `motiftile`, `isolines`, `text`, `weave`, `bank`, `ribbon`, `bloom`, `vine`), each a seeded `(rng, params) -> shapes[]` function. Up to 5 layers can be stacked, each with its own generator, density, scale, mix, and optional mask. The canvas size is not fixed: it is the operator's sheet minus the direct-SVG origin, fetched at startup. The sketch runs in an iframe (`/generative/index.html`) filling the canvas; the strip's `PRINT SKETCH` reads the frame on screen and sends it to the plotter. A `Stream to plotter` switch can auto-print each new frame on an interval — arming it opens a gate that estimates the current frame's plot time and requires an explicit `ARM STREAM`, because streaming draws real ink unattended.
...
- **Image → line art (`IMAGE` mode)** — upload a raster image (`src/neje_oracle/blocks/imaging/modes.py`) and convert it to single-pen polylines in one of nineteen modes: `trace` (follows the drawing's own strokes and fills bold ones to weight — for line art), `flow` (streamlines that wrap around form; best for photographs; `Dash mm` breaks streamlines into directional dashes), `crosshatch` (four layered angles), `hatch` (parallel lines only where dark; `Detail` = line spacing mm), `halftone` (dot size by darkness, shortest plot), `dither` (Floyd-Steinberg, highest fidelity/longest plot), `contour` (threshold-band outlines; `Detail` = band count), `spiral` (one wobbling spiral from the centre — fewest pen lifts of any mode), `wave` (rows rippling wider and faster with darkness; `Orientation` flips rows horizontal/vertical and `One line` joins them into a single stroke), `stipple` (density-weighted dots, chained within rows so pen lifts don't dominate), `squiggle` (one continuous meandering line, rows packing tighter where the image is dark), or one of the eight in `src/neje_oracle/blocks/imaging/art/`: `tsp` (stipple points joined into one travelling-salesman line), `ridgeline` (rows pushed up by darkness with hidden lines removed — the Unknown Pleasures look), `edges` (pencil-sketch outlines where brightness changes; best on photos), `hilbert` (one blocky space-filling curve, finer where dark), `ascii` (single-stroke characters chosen by measured ink), `truchet` (quarter-arc maze tiles, more arcs where dark), `rings` (halftone of hollow concentric circles), or `lowpoly` (jittered triangle facets, each hatched to its tone). `Cell mm` sets sampling resolution, `Gamma`/`Invert` adjust tone mapping. A `Pen lifts` slider caps how many pen lifts a plot may spend (slider at max = no limit): nearby strokes beyond the budget are joined with short connector lines, trading fidelity for plot time. Grid pitch is floored by the pen width, since a lattice finer than the nib fills in solid and detail finer than half a nib cannot print. The preview shows stroke/segment counts and an estimated plot time before `PRINT IMAGE` sends it to the plotter.
